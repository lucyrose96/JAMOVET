...
c2<-read.csv("c2.csv")
num_sims<-length(unique(c2$sim))
sims_vec=unique(c2$sim)


timetaken<-system.time(
  results <- foreach(i = sims_vec, .combine=rbind, .packages = c("rstan", "dplyr"))%dopar%{
    
    d                     <-data.frame(Total=c2[which(c2$sim == i),]$Total,
                                       C=c2[which(c2$sim == i),]$C,
                                       A=c2[which(c2$sim == i),]$A,
                                       fp=c2[which(c2$sim == i),]$fp_rate*c2[which(c2$sim == i),]$time_at_risk_years,
                                       rel_sens_asym=c2[which(c2$sim == i),]$rel_sens_asym,
                                       time_at_risk_years=c2[which(c2$sim == i),]$time_at_risk_years,
                                       vaccinated=c2[which(c2$sim == i),]$arm_01,
                                       intercept=c2[which(c2$sim == i),]$intercept
    )
    trial_dat_c2 <- list(N=nrow(d), 
                         Total=d$Total,
                         C=d$C,
                         A=d$A,
    ...
    as.data.frame(sampling(object = model, data = trial_dat_c2, chains = 4, cores = 4))%>%
...
write.csv(results, "results_c2_joint.csv", row.names = FALSE)
