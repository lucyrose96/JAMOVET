### Setup data for Rstan simulation model - no bias ###
...
#2. Simulate unbiased data
  ...
  c1_v<-data.frame(matrix(nrow = (num_subjects/2)*num_sims, ncol=4, data = NA ))
  colnames(c1_v)<-c("arm","A","C", "sim")
  c1_v$arm<-"V"
  c1_v$arm_01<-1
  c1_v$sim<-rep(1:num_sims, each =num_subjects/2)

  c1_p<-data.frame(matrix(nrow = (num_subjects/2)*num_sims, ncol=4, data = NA ))
  colnames(c1_p)<-c("arm","A","C", "sim")
  c1_p$arm<-"P"
  c1_p$arm_01<-0
  c1_p$sim<-rep(1:num_sims, each =num_subjects/2)

# c. simulate for vaccine and placebo arms
  c1_p<-c1_p%>%
  mutate(time_to_asym_years= rexp(nrow(c1_p), lam*(1-s)*sens_asym),
         time_to_fp_years=pmin(rexp(nrow(c1_p), num_tests*(1-spec_asym)), Inf, na.rm = TRUE),
         time_to_sym_years= rexp(nrow(c1_p), lam*s*sens_sym),
  ...
  c1_v<-c1_v%>%
  mutate(time_to_asym_years= rexp(nrow(c1_p), (1-VE_in)*lam*((1-s)+s*VE_pr)*sens_asym ),
         time_to_fp_years=pmin(rexp(nrow(c1_p), test_freq*(1-spec_asym)), Inf, na.rm = TRUE),
         time_to_sym_years= rexp(nrow(c1_p), (1-VE_in)*(1-VE_pr)*lam*s*sens_sym),
         time_at_risk_years= pmin(time_to_asym_years, time_to_fp_years, time_to_sym_years, follow_up),
         A=ifelse(time_to_asym_years == time_at_risk_years | time_to_fp_years == time_at_risk_years, 1, 0),
         C=ifelse(time_to_sym_years == time_at_risk_years, 1, 0),
         FP_a=ifelse(time_to_fp_years == time_at_risk_years, 1, 0),
...
# e. join datasets
  c1<-rbind(c1_p, c1_v)
  c1$fp_rate<-fp_rate
  c1$rel_sens_asym<-rel_sens_asym
  c1$intercept=1
  
  
  c1$outcome="None"
  c1$outcome[which(c1$FP_a == 1)]<-"False asymptomatic"
  c1$outcome[which(c1$TP_a == 1)]<-"True asymptomatic"
  c1$outcome[which(c1$C == 1)]<-"True symptomatic"
...
write.csv(c1, "c1.csv", row.names = FALSE)
